...
struct Alert {
    ...
    init(titleKey: String, messageKey: String, acceptAction: Action, cancelAction: Action?) {
...
class AlertsService {
    ...
    func runAlert(on viewController: UIViewController, alert: Alert) {
        ...
        if let cancelAction = alert.cancelAction {
            ...
            let canccelHandler: (UIAlertAction) -> Void = { _ in cancelAction.handler?()}
            let cancelButton = UIAlertAction(title: cancelButtonTitle, style: cancelAction.style, handler: canccelHandler)
